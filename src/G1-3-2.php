...
<?php
    ...
    $sql=$pdo->prepare('update Shop set shop_mei=?,postcode=?,address=?,tel=?,imgpath=?,koshinbi=CURRENT_DATE where shop_id=?');
    ...
    }else if($sql->execute([$_POST['shop_mei'],$_POST['postcode'],$_POST['address'],$_POST['tel'],$_POST['imgpath'],$_POST['shop_id']])){
        echo '<font color="red" font size=30px>更新に成功しました。</font>';
    }else {
        echo '<font color="red" font size=30px>更新に失敗しました。</font>';
    }
?>
<br><br>
